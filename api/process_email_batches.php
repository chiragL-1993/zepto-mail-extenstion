...
foreach ($batches as $batch) {
    ...
    foreach ($chunks as $chunk) {
        ...
        foreach ($records as $record) {
            ...
            $emailModuleConfiguration['mobile_number'] = $moduleRecords->{$moduleId};
            ...
            // Handle Errors from Zepto Mail API
            if ($emailResult->error->code !== "SUCCESS") {
                $msg = $emailResult->error->description && $emailResult->error->description->reason
                    ? "Error from Zepto Email - " . $emailResult->error->description->reason
                    : "Unknown failure response from Zepto Email";
                if (
                    !empty($emailResult->error) &&
                    !empty($emailResult->error->code) &&
                    ($emailResult->error->code == "LEDGER_ERROR" || $emailResult->error->code == "RECIPIENTS_ERROR")
                ) {
                    $clientSc->log->warning($msg, [
                        'emailResult' => $emailResult,
                        'emailParameters' => $emailParameters,
