...
/**
 * process_schedule_bulk_email.php - Processes Email batches to send bulk Email (immediate and scheduled)
 *
 *
...
const CONTEXT = "Process Scheduled Email Batches";
...
$squirrelSc->log->info('Scheduled Batch Info: ' . json_encode($batches));

foreach ($batches as $batch) {
    // Verify to see if it's been picked up by another instance of the Job
    $refreshedBatch = ZeptoEmailBatches::find($batch->id);
    if ($refreshedBatch->status != BATCH_PROCESSING_STATUSES['scheduled']) {
        continue;
    }
    ...
    foreach ($chunks as $chunk) {
        ...
        foreach ($records as $record) {
            ...
            $dbEmailRecord = (new ZeptoEmailRecords)->create([
                ...
                'batch_id' => $batch->id,
                'status' =>  EMAIL_HISTORY_STATUSES['sent']
            ]);
